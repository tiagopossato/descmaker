...
def descmaker_espidf_builder(supervisors, global_event_list, base_dir, output_dir):
    ...
    for event in global_event_list:
        ...
        events_c += (
            f"Event {event['Name']} = {{{event['Kind']}, {i}, "
            f"SUP_DEBUG_STR(\"{event['Name']}\"), NULL}};\n"
            )
    ...
    fill_template(
        f"{base_dir}/templates_esp-idf/template/components/event_handler/events-template.h",
    ...
    fill_template(
        f"{base_dir}/templates_esp-idf/template/components/event_handler/events-template.c",
    ...
    fill_template(
        f"{base_dir}/templates_esp-idf/template/main/main-template.c",
    ...
    for sup_index in range(len(supervisors)):
        ...
        next_sup = (
            f"&{supervisors[sup_index+1]['name']}_list"
            if sup_index < len(supervisors)-1 else "NULL"
        )
        if index == 0:
            supervisor_list_init += f"SupervisorList sup_list = {{&{sup['name']}, {next_sup}}};\n"
        else:
            supervisor_list_init += (
                f"SupervisorList {sup['name']}_list = {{&{sup['name']}, {next_sup}}};\n"
                )
        ...
        for i in range(len(sup['event_list'])):
            ...
            next_event_name = (
                f"&{sup['name']}_{sup['event_list'][i+1]['Name']}_evt{i+1}"
                if i < len(sup['event_list'])-1  else 'NULL'
                )
            alphabet_init += (
                f"const Alphabet {sup['name']}_{event['Name']}_evt{i} = "
                f"{{&{event['Name']}, {next_event_name}}};\n"
                )
            i=i+1
        ...
        for i in range(len(sup['transition_list'])):
            ...
            next_transition = (
                sup['transition_list'][i+1] if i < len(sup['transition_list'])-1 else None
            )
            ...
            if state_initializer_dict[state_name] is None:
                ...
                for state in sup['state_list']:
                    ...
                    if state['Name'] == transition['Source']:
                        ...
                        state_initializer_dict[state_name] = (
                            f"const State {state_name} = {{{is_initial}, "
                            f"SUP_DEBUG_STR(\"{transition['Source']}\"), &{transition_name}}};\n"
                        )
                        break
            ...
            if next_transition is not None:
                if state_initializer_dict[f"{sup['name']}_{next_transition['Source']}"] is None:
                    ...
                    next_name = (
                        f"&{sup['name']}_{next_transition['Source']}_t{j+1}"
                        if next_transition else 'NULL'
                    )
            else:
                next_name = 'NULL'
            ...
            transition_init = (
                f"const Transition {transition_name} = "
                f"{{&{transition['Event']}, &{sup['name']}_{transition['Target']}, {next_name}}};\n"
            )
        ...
        fill_template(
            f"{base_dir}/templates_esp-idf/template/components/supervisors/supervisor-template.c",
        ...
        fill_template(
            f"{base_dir}/templates_esp-idf/template/components/supervisors/supervisor-template.h",
    ...
    fill_template(
        f"{base_dir}/templates_esp-idf/template/components/supervisors/supervisor_list-template.c",
    ...
    fill_template(
        f"{base_dir}/templates_esp-idf/template/components/supervisors/CMakeLists-template.txt",
